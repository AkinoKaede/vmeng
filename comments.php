            ...
            <?php else : ?>
                <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" class="commentform" id="commentform">
                    <?php if ( $user_ID ) : ?>
                        ...
                        <textarea class="form-control OwO-textarea" rows="3" id="comment" onkeydown="if(event.ctrlKey&event.keyCode==13){document.getElementById('submit').click();return false};" placeholder="快来Biu一发~(｡>∀<｡)" class="form-control" tabindex="1" name="comment"></textarea>
                        <div class="OwO"></div>
					<?php else : ?>
                        <textarea class="form-control OwO-textarea" rows="3" id="comment" onkeydown="if(event.ctrlKey&event.keyCode==13){document.getElementById('submit').click();return false};" placeholder="快来Biu一发~(｡>∀<｡)" tabindex="1" name="comment"></textarea>
                        <div class="OwO"></div>						
					...
					<div class="btn-group commentBtn" role="group">
						...
						<p></p>
						<span class="mail-notify"> <input type="checkbox" name="comment_mail_notify" id="comment_mail_notify" checked="checked" class="notify" value="comment_mail_notify"> <label for="comment_mail_notify"><span>有回复时邮件通知我</span></label> </span>
